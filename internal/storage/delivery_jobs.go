...
func (r *Repository) ClaimDeliveryJobs(ctx context.Context, workerID string, limit int, lockFor time.Duration) ([]domain.DeliveryJob, error) {
	...
	rows, err := r.pool.Query(ctx, `
		...
		SET status = 'processing', locked_by = $1, locked_until = NOW() + ($2::int * INTERVAL '1 second'), updated_at = NOW()
		WHERE id IN (
			SELECT id
			FROM delivery_jobs
...
func (r *Repository) MarkDeliveryJobFailed(ctx context.Context, jobID int64, errText string, retryAfter time.Duration) error {
	...
	tag, err := r.pool.Exec(ctx, `
		...
		SET attempts = attempts + 1,
		    ...
		    next_attempt_at = CASE WHEN attempts + 1 >= max_attempts THEN next_attempt_at ELSE NOW() + ($2::int * INTERVAL '1 second') END,
		    locked_until = NULL,
		    locked_by = NULL,
		    last_error = $3,
		    updated_at = NOW()
